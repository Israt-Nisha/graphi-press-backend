...
const changeUserStatus = async (user : IRequestUser, payload : IChangeUserStatusPayload) => {
    ...
    // 1. Admin can change the status of admin designer and customer. Except himself. He cannot change his own status.
    ...
    if (userToChangeStatus.status === UserStatus.DELETED) {
        throw new AppError(status.BAD_REQUEST, "User is already deleted and cannot be updated.");
    }
...
const changeUserRole = async (user : IRequestUser, payload : IChangeUserRolePayload) => {
    ...
    if(userToChangeRole.role === Role.DESIGNER || userToChangeRole.role === Role.CUSTOMER){
        throw new AppError(status.BAD_REQUEST, "You cannot change the role of designer or customer user. If you want to change the role of designer or customer user, you have to delete the user and recreate with new role");
    }
